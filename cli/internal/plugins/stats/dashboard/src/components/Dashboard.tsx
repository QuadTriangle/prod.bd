...
export default function Dashboard() {
  ...
  const [baseDomain, setBaseDomain] = useState('.prod.bd');
  ...
  useEffect(() => {
    fetch('/api/stats/account/config').then(r => r.json()).then((cfg: any) => {
      if (cfg.base_domain) setBaseDomain(cfg.base_domain);
    }).catch(() => {});
  }, []);
